...
const fetchCartsDb = async () => {
        const res = await pool.query(
                `SELECT * FROM carts
                INNER JOIN cart_products ON cart.id = cart_products.cart_id
...
const fetchCartByIdDb = async (userId) => {
        const res = await pool.query(
                `SELECT products.id, name, description, category, image_url, status, quantity FROM carts
...
const createProductInCartDb = async ({cart_id, product_id, quantity}) => {
        const text = `INSERT INTO cart_products (cart_id, product_id, quantity)
...
const modifyCartDb = async ({cart_id, product_id, quantity}) => {
        const text =    `UDPATE cart_products
                        SET quantity = $3
