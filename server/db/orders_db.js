...
const fetchOrdersByUserDb = async (userId) => {
        const res = await pool.query(`SELECT orders-id AS irder_id, products.id AS product_id, products.name, products.price, quantity, orders.created_at FROM orders
                INNER JOIN order_products ON orders.id = order_products.order_id
                INNER JOIN products ON order_products.products_id = products.id
...
const createProductInOrderDb = async ({order_id, product_id, quantity, price}) => {
        const text = `INSERT INTO order_products (order_id, product_id, quantity, price)
