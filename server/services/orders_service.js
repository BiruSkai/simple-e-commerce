const { fetchOrdersDb, fetchOrderByIdDb, fetchOrdersByUserDb, createOrderDb, 
        createProductInOrderDb, fetchCartByIdDb } = require("../db")
...
const calculateOrderAmount = async (userId) => {
        // Get price fromc arts in db and not from items (for now)
        const cart = await fetchCartByIdDb(userId)
