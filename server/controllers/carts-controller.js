const { cartsService, orderService } = require("../services")
...
//Get carts from db, adds items from logged-out cart and sends updated cart back
const syncCartSelf = async (req, res, next) => {
        const userId = req.user.id //Extract user id from passport 'login' object
        const cartId = req.user.cart_id //Extract cart_id from passport 'login' object
        const dbCart = await fetchCartById(userId)
        const loggedOutCart = req.body.cart
        console.log("carts-controller.js re.body.cart: ", loggedOutCart)
